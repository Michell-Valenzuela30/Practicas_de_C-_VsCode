...
//CLASE CLIENTE: SE CREA LA CLASE CLIENTE, Y DEFINIMOS LOS ATRIBUTOS Y M TODOS
class Persona{
	public:
		char nombre[35];
		char dni[15];
...
void menu(){
	Cliente Cl; //AQU  SE INSTANCIA LA CLASE CLIENTE, ES DECIR, SE CREA EL OBJETO LLAMADO CL , CON ESTO SE PODR  LLAMAR A LOS M TODOS QUE TIENE LA CLASE CLIENTE 
	char op,op2;
	limpia();
	CursorOn(1,10);
	do{
	...
	if (op=='1'){
		...
		if (op2=='1') Cl.ingreso();   //AQU  SE LLAMA AL M TODO INGRESO
		if (op2=='2') Cl.actualizar();   //AQU  SE LLAMA AL M TODO ACTUALIZAR
		if (op2=='3') Cl.eliminar();     //AQU  SE LLAMA AL M TODO ELIMINAR
		if (op2=='4') Cl.consultar();    //AQU  SE LLAMA AL M TODO CONSULTAR
		if (op2=='5') menu();
	}	
	...
	if (op=='2'){
		...
		if (op2=='1') comprar();         //AQU  SE LLAMA AL M TODO CONSULTAR
		if (op2=='2') menu();
	}
...
// FUNCION INGRESO DE DATOS: ESTA FUNCI N PERMITE INGRESAR LOS DATOS DEL CLIENTE, Y LA CREACI N DEL ARCHIVO QUE LO LLAMAREMOS E2, QUE ALMACENAR  ESTOS DATOS PARA QUE UNA VEZ SE CIERRE EL PROGRAMA, ESTOS QUEDEN GUARDADOS.
void Cliente::ingreso(){
	...
	Cliente nuevo, lista[50];                      //AQU  SE CREAN DOS OBJETOS DE LA CLASES CLIENTE, UNO ES NUEVO Y EL OTRO ES UN ARRAY O VECTOR LLAMADO LISTA..
	limpia();                                      //..EL OBJETO NUEVO NOS PERMITER  ALMACENAR LOS DATOS EN LAS VARIABLES, PARA LUEGO GUARDARLOS EN EL ARCHIVO, M S ABAJO SE INDICE LA LINEA DONDE SE GUARDAN.
	...
	do{
		...
		//PROCESO DE B SQUEDA DE LA INFORMACI N
		ultimo=res-1;
		i=0;
		encontro=1;
...
//FUNCION QUE MUESTRA VENTANA DE LOGIN , Y PERMITE INGRESAR USUARIO Y CONTRASE A
void login(){
	CursorOn(1,10);
	char user[] = "Admin"; //Modificar colocando el usuario que deseean
	char contra[] = "12345"; //Modificar contrase a que deseen
...
//FUNCION GOTOXY, SIRVE PARA POSICIONAR LOS MENSAJES EN UNA DETERMINADA ZONA DE LA PANTALLA, SE INGRESA COLUMNA Y FILA DE LA PANTALLA, EJEMPLO: gotoxy(4,10); printf("HOla"); ESTE MENSAJE SE IMPRIMIR  EN LA POSICI N COLUMNA 4, FILA 10
void gotoxy(int x,int y){  
      HANDLE hcon;  
